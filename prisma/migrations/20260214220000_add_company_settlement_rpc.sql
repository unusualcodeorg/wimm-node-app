-- Migration: Add company settlement RPC
-- Created: 2026-02-14
-- Description:
--   Adds company_settlement(order_id, client_id) to settle OPEN orders
--   as CREDIT/PENDING against a selected client account.
...
BEGIN
  ...
  FROM public.payment_methods pm
  WHERE pm.name = 'PENDING'
  LIMIT 1;

  IF v_pending_status_id IS NULL THEN
    RAISE EXCEPTION 'PENDING payment method is not configured';
  END IF;
