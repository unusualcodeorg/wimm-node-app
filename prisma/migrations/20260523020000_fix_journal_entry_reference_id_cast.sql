-- Migration: Fix incorrect ::TEXT cast on journal_entries.reference_id (which is UUID)
-- The previous migration passed p_invoice_id::TEXT / v_payment_id::TEXT into a UUID column.
...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      ...
      -- inventory_stock_movements.reference_id is TEXT
    ...
    IF v_inventory_account_id IS NOT NULL AND v_ap_account_id IS NOT NULL THEN
      ...
      -- journal_entries.reference_id is UUID — pass p_invoice_id directly (no ::TEXT cast)
    ...
    -- financial_event_logs.reference_id is UUID — pass p_invoice_id directly
...
BEGIN
  ...
  BEGIN
    ...
    IF v_ap_account_id IS NOT NULL AND v_cash_account_id IS NOT NULL THEN
      ...
      -- journal_entries.reference_id is UUID — pass v_payment_id directly (no ::TEXT cast)
      INSERT INTO public.journal_entries (
        company_id, outlet_id, posting_scope,
        reference_type, reference_id,
        source_module, entry_date, memo, status, created_by
    ...
    -- financial_event_logs.reference_id is UUID — pass v_payment_id directly
    INSERT INTO public.financial_event_logs (
      company_id, outlet_id, source_module, event_type,
      reference_type, reference_id,
      idempotency_key,
