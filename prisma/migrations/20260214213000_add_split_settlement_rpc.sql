-- Migration: Add split settlement RPC
-- Created: 2026-02-14
-- Description:
--   Adds split_settlement(order_id, payments_json) to settle OPEN orders
--   across multiple payment methods in a single request.

CREATE OR REPLACE FUNCTION public.split_settlement(
  p_order_id UUID,
  p_payments JSONB,
  p_notes TEXT DEFAULT NULL
)
RETURNS SETOF public.order_settlements
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_sum_amount NUMERIC(12, 2) := 0;
  v_first_payment_method_id INTEGER := NULL;
BEGIN
  v_user_id := auth.uid();
  ...
  IF p_payments IS NULL OR jsonb_typeof(p_payments) <> 'array' OR jsonb_array_length(p_payments) = 0 THEN
    RAISE EXCEPTION 'At least one split payment entry is required';
  END IF;
  ...
  IF v_order_status_id <> v_open_status_id THEN
    RAISE EXCEPTION 'Only OPEN orders can use split settlement';
  END IF;
  ...
  LOOP
    v_payment_method_id := NULLIF(TRIM(v_payment->>'payment_method_id'), '')::INTEGER;
    v_amount := ROUND(COALESCE(NULLIF(TRIM(v_payment->>'amount'), '')::NUMERIC, 0), 2);

    IF v_payment_method_id IS NULL THEN
      RAISE EXCEPTION 'Split payment has invalid payment_method_id';
    END IF;
    ...
    IF v_payment_method_name IS NULL THEN
      RAISE EXCEPTION 'Invalid payment method id: %', v_payment_method_id;
    END IF;

    IF v_payment_method_name NOT IN ('CASH', 'CARD', 'MOBILE MONEY', 'CHECK', 'EFT') THEN
      RAISE EXCEPTION 'Unsupported split settlement payment method: %', v_payment_method_name;
    END IF;

    IF v_first_payment_method_id IS NULL THEN
      v_first_payment_method_id := v_payment_method_id;
    END IF;

    v_sum_amount := v_sum_amount + v_amount;
  END LOOP;

  IF v_sum_amount <= 0 THEN
    RAISE EXCEPTION 'At least one split amount must be greater than zero';
  END IF;

  IF ROUND(v_sum_amount, 2) <> ROUND(v_net_total, 2) THEN
    RAISE EXCEPTION 'Split amounts must equal bill total (expected %, got %)', v_net_total, v_sum_amount;
  END IF;

  RETURN QUERY
  WITH payments AS (
    SELECT
      NULLIF(TRIM(value->>'payment_method_id'), '')::INTEGER AS payment_method_id,
      ROUND(COALESCE(NULLIF(TRIM(value->>'amount'), '')::NUMERIC, 0), 2)::NUMERIC(12, 2) AS amount
  ...
  SET
    status_id = COALESCE(v_first_payment_method_id, status_id),
    updated_at = now()
  WHERE id = p_order_id;
END;
$$;

GRANT EXECUTE ON FUNCTION public.split_settlement(UUID, JSONB, TEXT) TO authenticated;
