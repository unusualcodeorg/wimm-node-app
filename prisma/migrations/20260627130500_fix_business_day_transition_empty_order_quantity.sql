-- Migration: Fix business day transition to evaluate true item quantities for empty orders
-- Created: 2026-06-27
-- Description:
--   Updates the business day transition logic to evaluate SUM(quantity) instead of COUNT(*).
--   This ensures orders with zero-quantity line items are correctly classified as empty and auto-canceled.

-- 1. Drop existing function to cleanly alter return table signature variations
DROP FUNCTION IF EXISTS public.transition_outlet_business_day(UUID, UUID, DATE);

-- 2. Create updated function implementation
...
BEGIN
  ...
  SELECT
    COALESCE(COUNT(*) FILTER (WHERE total_quantity > 0), 0)::INTEGER,
    COALESCE(
      ARRAY_AGG(order_id) FILTER (WHERE total_quantity = 0),
      ARRAY[]::UUID[]
    )
  INTO
