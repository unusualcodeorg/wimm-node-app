-- Migration: Fix settle_credit_bill reversal journal entry ordering
-- Created: 2026-05-28
-- Description:
--   The BEFORE INSERT trigger (validate_journal_entries_before_posting) fires
--   when a journal entry is inserted with status = 'posted' and validates that
--   at least one line exists.  The original implementation inserted the reversal
--   header as 'posted' and then inserted lines — the trigger fired before the
--   lines existed and raised "Journal entry must contain at least one line
--   before posting."
--
--   Fix: insert the reversal header as 'draft', insert lines, then UPDATE
--   the header to 'posted'.  The trigger re-fires on the UPDATE and by then
--   the lines are present.
...
BEGIN
  ...
  IF v_old_je_id IS NOT NULL THEN
    -- Insert reversal header as 'draft' so the BEFORE INSERT trigger does
    -- not fire the balance/line-count validation before lines are added.
    ...
    -- Post the reversal entry now that lines exist; the BEFORE UPDATE
    -- trigger re-runs validate_journal_entry_can_post and passes.
