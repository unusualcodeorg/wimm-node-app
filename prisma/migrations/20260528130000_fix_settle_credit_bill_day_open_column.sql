-- Migration: Fix settle_credit_bill day_open column name
-- Created: 2026-05-28
-- Description:
--   The outlets table uses day_opened_at (date), not day_open.
--   The previous migration referenced outl.day_open which does not exist,
--   causing "column outl.day_open does not exist" when settling a credit bill.
...
BEGIN
  ...
  IF v_old_je_id IS NOT NULL THEN
    -- NOTE: superseded by migration 20260528140000 which uses the correct
    -- 4-step deferred-link pattern from reverse_journal_entry(). This block
    -- is kept for migration history completeness but is no longer executed.

    -- Step 1: Insert header as 'draft' WITHOUT reversal_of_journal_entry_id.
    ...
    -- Step 3: Post the reversal AND set reversal_of_journal_entry_id in one UPDATE.
    ...
    -- Step 4: Mark original entry as reversed (posted → reversed transition).
    UPDATE public.journal_entries
    SET
